...
<div class="header-box">REPORTE DE FACTURAS</div>
...
<?php
$q = "SELECT f.folio, c.nombre, TIME(f.fecha) AS hora, f.cfdi
      FROM t_facturas f
      JOIN t_vender_general vg ON f.id_vg = vg.id
...
if($r && mysqli_num_rows($r)>0){
    while($row=mysqli_fetch_array($r)){
        echo "<div class='fila'>";
        echo "<span>#".$row['folio']."</span>";
        echo "<span>".htmlspecialchars($row['nombre'])."</span>";
        echo "<span>".$row['hora']."</span>";
        echo "<span>".htmlspecialchars($row['cfdi'])."</span>";
        echo "</div>";
    }
}else{
    echo "<div class='sin-datos'>Sin facturas</div>";
}
?>
</div>
