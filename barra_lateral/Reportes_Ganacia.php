...
$qIng = "SELECT SUM(vp.subtotal) AS ingresos
         FROM t_vender_general vg
         JOIN t_vender_particular vp ON vg.id = vp.id_vg
         WHERE MONTH(vg.fecha)=$mes AND YEAR(vg.fecha)=$anio";

$ingresos = mysqli_fetch_array(mysqli_query($link,$qIng))['ingresos'] ?? 0;

$qCosto = "SELECT SUM(pp.costo * pp.cantidad) AS costos
           FROM t_proporcionar_general pg
           JOIN t_proporcionar_particular pp ON pg.id = pp.id_pg
           WHERE MONTH(pg.fecha)=$mes AND YEAR(pg.fecha)=$anio";

$costos = mysqli_fetch_array(mysqli_query($link,$qCosto))['costos'] ?? 0;
...
<div class="header-box">REPORTE DE GANANCIAS</div>
...
<div class="result-box">
    ...
    <div class="valor">$<?php echo number_format($ganancia,2); ?></div>
</div>
